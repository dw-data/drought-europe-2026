...
\| [Spanish](https://www.dw.com/es/a-78650790) \| [German](https://www.dw.com/es/a-xxx)
...
hh_share = wat_cat %>% 
  ...
  #append population
  left_join(pop %>% rename(pop = values),
            by = c("geo" = "geo", "time" = "time")) %>%
  #household use in l/day/person
  mutate(values_pp = values * 10^6 / pop * 1000 / 365) %>%
  #calculate shares
  group_by(geo) %>% 
  mutate(share = values / sum(values), n = n()) %>% 
  #filter for household data, only countries that have totals
  filter(n == 2, nace_r2 == "EP_HH", !is.na(values_pp)) %>% 
  select(-wat_proc, -n, -nace_r2, -values) %>% ungroup %>% 
  #calculate x values for bubble chart, append country names
  mutate(offset = vipor::offsetX(values_pp),
         country = label_eurostat(geo, "geo"))

write.csv(hh_share, "data/processed/hh_share.csv",
          row.names = FALSE, na = "")

hh_share %>% select(1,4,5) %>%
  mutate(share = round(share*100) %>% paste0(.,"%")) %>%  
...
eea_total %>% select(1,3) %>% knitr::kable(digits = 0)
```
...
eea_countries = eea %>% 
  ...
  left_join(hh_share %>% select(1,5), by = c("code" = "geo")) %>% 
